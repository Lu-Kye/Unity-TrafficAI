...
public class TrafficAIExporter
{
	...
	static string GetNodeFilePath()
	{
		...
		targetPath = Path.Combine(targetPath, ResourceConfig.JSON_SG_TRAFFIC_NODE.Path);
	...
	static string GetEdgeFilePath()
	{
		var assetPath = Application.dataPath;
		var targetPath = Path.Combine(assetPath, "Resources");
		targetPath = Path.Combine(targetPath, ResourceConfig.JSON_SG_TRAFFIC_EDGE.Path);
		return targetPath + ".json";
	}
	#endregion
